...
import { OpcionEmergencia, TipoEmergencia } from '../../core/models/emergencia.model';
...
export class DetalleCategoriaPage {
  ...
  get titulo(): string {
    if (this.tipo === 'DELITOS_GRAVES') return 'Delitos Graves';
    if (this.tipo === 'ASISTENCIA_VIAL') return 'Asistencia / Vial';
    return 'Sospecha y Prevención';
  }

  get claseColor(): string {
    if (this.tipo === 'DELITOS_GRAVES') return 'red';
    if (this.tipo === 'ASISTENCIA_VIAL') return 'orange';
    return 'blue';
  }
  ...
  async seleccionar(opcion: OpcionEmergencia): Promise<void> {
    ...
    if (this.emergenciaId) {
      this.emergenciaService.completarDetalle(this.emergenciaId, {
        ...
        next: emergencia => this.router.navigate(['/alerta-activa', emergencia.id]),
        error: () => {
          this.enviando = false;
    ...
    this.emergenciaService.crear({
      ...
      error: () => {
        this.enviando = false;
        alert('No se pudo crear la emergencia.');
      }
    });
  }
