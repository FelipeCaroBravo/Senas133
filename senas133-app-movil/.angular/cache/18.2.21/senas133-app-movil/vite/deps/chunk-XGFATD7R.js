...
var r2 = (o, s2, e2, r3, i2, l2, f, p, d) => {
  ...
  const k = a(r3).querySelector("ion-icon").getBoundingClientRect(), w = s2 ? k.width / 2 - (k.right - i2.right) + "px" : i2.left - k.width / 2 + "px", T = s2 ? `-${window.innerWidth - i2.right}px` : `${i2.left}px`, A = `${d.top}px`, j = `${i2.top}px`, z = e2 ? [{
    offset: 0,
    transform: `translate3d(${T}, ${j}, 0)`
  }, {
    offset: 1,
    transform: `translate3d(${w}, ${A}, 0)`
  }] : [{
    offset: 0,
    transform: `translate3d(${w}, ${A}, 0)`
  }, {
    ...
    opacity: 1,
    transform: "scale(1)"
  }], I = r(), N = r(), O = r(), S = n("ion-back-button"), R = a(S).querySelector(".button-text"), q = a(S).querySelector("ion-icon");
  S.text = r3.text, S.mode = r3.mode, S.icon = r3.icon, S.color = r3.color, S.disabled = r3.disabled, S.style.setProperty("display", "block"), S.style.setProperty("position", "fixed"), N.addElement(q), I.addElement(R), O.addElement(S), O.beforeStyles({
    position: "absolute",
    top: "0px",
    [X]: "0px"
  }).beforeAddWrite(() => {
  ...
  }).keyframes(z), I.beforeStyles({
    "transform-origin": `${m} top`
  }).keyframes(B2), N.beforeStyles({
    "transform-origin": `${y} center`
  }).keyframes(C), o.addAnimation([I, N, O]);
};
var i = (o, a2, s2, e2, r3, i2, l2, f, p) => {
  ...
  k.innerText = e2.innerText, k.size = e2.size, k.color = e2.color, w.addElement(k), w.beforeStyles({
